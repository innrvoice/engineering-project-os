...
def prepare(case: str, target: Path) -> None:
    ...
    connected = {"check-valid-connected-repository", "upgrade-current-repository-is-idempotent",
                 "private-lesson-is-not-promoted", "resume-blocked-plan-after-condition-clears"}
    if case not in connected:
        return
    project_os.init_project(target, "auto", "auto", False)
    creates = []
    updates = {}
    if case == "private-lesson-is-not-promoted":
        lesson = {
            "id": "REVIEW-PRIVATE-001", "title": "Synthetic private lesson", "status": "active",
            "applies_to": ["review fixture"], "trigger": "A retry occurs after the owner changes.",
            "mechanism": "A late result is applied to a new owner. Private context: <PRIVATE_PERSON>, <PRIVATE_PATH>, https://example.invalid/evidence?signature=synthetic.",
